...
;(function($, window, undefined) {
  ...
  var defaults = {
    ...
    stop: function(){},
    rest: function() {},
    easing: null, // called while object is easing

    moveTo: false, // custom method to override the default moveTo functionality.

    constrainTo: 'parent',

    disableSelect: true, // add 'user-select: none;' to $el.
    removeMargins: true, // remove margins for better object placement
    ignoreRightClick: false, // start event will be ignored if triggered by a right click

    elementsWithInteraction: 'input', // valid CSS/jQuery selector for elements within the Throw object that should allow user interaction, and thus propagate to allow movement
    hardwareAccelerate: true,

    activeClass: 'throw-active',
  ...
  function Throw( element, options ) {
    ...
    // setting variables. use only once
    ...
    this.initialPosition = { top: 0, left: 0 };
  ...
  Throw.prototype.init = function() {
    if ( this.options.disableSelect ) this.disableSelect();
    
    // position the parent and object
    this.positionParent();
    this.placeObject();
  ...
  // useful in the event we want to programmatically interact with our Throw object.
  Throw.prototype.subscribe = function () {
    var self = this;
    ...
    this.$doc.on(this.stopEvents, this.onStopEvents);

    // Subscribe to our move event
    this.onMoveEvents = function(ev){
      self.moveEvent = ev;
    };
    this.$doc.on(this.moveTrigger, this.onMoveEvents);
  };
  ...
  Throw.prototype.handleStart = function(ev) {
    // ignorePropagation is set to true if the event originates from an element listed in this.options.elementsWithInteraction
    ...
    if ( this.isValidMoveEvent(ev) && !this.disabled ){
      if( !(this.options.ignoreRightClick && ev.which === 3) ) {
        ...
        // TEST
        // animation loop to ensure we don't fire too many unneccessary repaints
  ...
  Throw.prototype.handleMove = function() {
    ...
    // last in, first out (LIFO) stack to help us manage velocity
  ...
  Throw.prototype.handleStop = function(ev) {
    ...
    this.$el.removeClass('throw-start')
            .addClass('throw-ease');
    ...
    */
    ...
    // reset the velocity stack
    this.resetVelocityStack();
  };

  // Removes the active class.
  Throw.prototype.removeActiveClass = function() {
    this.$el.removeClass( [this.options.activeClass, 'throw-ease'].join(' ') );
  };

  // used in conjunction with the LIFO stack to ease the object after stop
  Throw.prototype.ease = function(ev, started){
    console.log('ev, started :', ev, started);
  ...
  Throw.prototype.velocity = function(){
    var sumX = 0;
    var sumY = 0;

    for ( var i = 0; i < this.velocityStack.length - 1; i++ ) {
      if ( this.velocityStack[i] ){
        sumX        += (this.velocityStack[i+1].x - this.velocityStack[i].x);
        sumY        += (this.velocityStack[i+1].y - this.velocityStack[i].y);
        this.dt     = ( this.velocityStack[i+1].time - this.velocityStack[i].time );
      }
    }
    console.log('velocity :', sumX, sumY);

    // return velocity in each direction.
    return { x: sumX * this.options.velocityMultiplier, y: sumY * this.options.velocityMultiplier};
  };

  Throw.prototype.doMoveTo = function(dx, dy) {
    ...
    if ( typeof this.options.moveTo === 'function') {
      ...
      // console.log('doMoveTo() - dx, dy, hash :', dx, dy, hash);
  ...
  // move the object to an x and/or y value using jQuery's .css function -- this fxn uses the .css({top: "+=20", left: "-=30"}) syntax
  Throw.prototype.moveTo = function(x,y, animate) {
    if ( animate ) {
  ...
  Throw.prototype.moveToUsingTransforms = function(x,y) {
  ...
  Throw.prototype.xTranslation = function(matrixArray) {
    matrixArray  = matrixArray || this.matrixToArray( this.matrixString() );
    return parseInt(matrixArray[4], 10);
  };

  Throw.prototype.yTranslation = function(matrixArray) {
    matrixArray  = matrixArray || this.matrixToArray( this.matrixString() );
    return parseInt(matrixArray[5], 10);
  };

  Throw.prototype.transform = function(value) {
    this.$el.css({
  ...
  // 3 helper functions for working with the objects CSS3 transforms
  // matrixString
  // matrixToArray
  // arrayToMatrix
  Throw.prototype.matrixString = function() {
  ...
  Throw.prototype.matrixToArray = function(str) {
      return str.split('(')[1].split(')')[0].split(',');
  };

  Throw.prototype.arrayToMatrix = function(array) {
      return "matrix(" +  array.join(',')  + ")";
  };

  // returns a hash of where to move to when we constrain to parent/window
  Throw.prototype.handleConstraint = function(dx, dy, accountForTranslation) {
    var pos = this.$el.position();
    this.pos.x = pos.left;
    this.pos.y = pos.top;
  ...
  // a Last-In/First-Out array of the 5 most recent velocity points, which is used for easing
  Throw.prototype.addToLIFO = function(val){
    // last in, first out
  ...
  Throw.prototype.removeCSSEasing = function() {
    if ( this.cssAnimationsSupported() ) this.$el.css( this.getCSSEaseHash(true) );
  };

  // returns true if the browser supports CSS animations which are used for easing..
  Throw.prototype.cssAnimationsSupported = function() {
    ...
    if( animation === false ) {
      for( var i=0,max=domPrefixes.length; i<max; i++ ) {
  ...
  Throw.prototype.shouldUseCSSTranslation = function() {
  ...
  Throw.prototype.requestAnimationFrame = function(callback) {
  ...
  Throw.prototype.hardwareAccelerate = function() {
    this.$el.css({
  ...
  //    return whether or note our device is pointer event compatible; typically means where on a touch Win8 device
  Throw.prototype.isPointerEventCompatible = function() {
    return ('MSPointerEvent' in window);
  };

  // normalizeEvent()
  Throw.prototype.normalizeEvent = function(ev) {
      ev.pep = {};
  ...
  Throw.prototype.isValidMoveEvent = function(ev){
    return ( !this.isTouch(ev) || ( this.isTouch(ev) && ev.originalEvent && ev.originalEvent.touches && ev.originalEvent.touches.length === 1 ) );
  };

  Throw.prototype.isTouch = function(ev){
    return ev.type.search('touch') > -1;
  };


  // add the relative positioning to the parent object
  Throw.prototype.positionParent = function() {
    if ( this.parentPositioned ) return;
  ...
  Throw.prototype.placeObject = function() {
    if ( this.objectPlaced ) return;
    this.objectPlaced = true;

    this.offset = (this.options.constrainTo === 'parent' ) ? this.$el.position() : this.$el.offset();
  ...
  Throw.prototype.disableSelect = function() {
    this.$el.css({
  ...
  Throw.prototype.resetVelocityStack = function() {
    this.velocityStack = new Array(5);
  };

  Throw.prototype.getCSSEaseHash = function(reset){
  ...
  $.fn[PLUGIN_NAME] = function ( options ) {
    return this.each(function () {
      if (!$.data(this, 'plugin_' + PLUGIN_NAME)) {
        var pepObj = new Throw( this, options );

        $.data(this, 'plugin_' + PLUGIN_NAME, pepObj);
